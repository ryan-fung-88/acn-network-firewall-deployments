...
resource "aws_vpc" "this_inspection_vpc" {
    cidr_block = var.cidr
...
resource "aws_subnet" "this_inspection_public_subnets" {
    ...
    vpc_id = aws_vpc.this_inspection_vpc.id
    enable_dns64 = var.enable_public_dns64
    map_public_ip_on_launch = var.map_public_ip_on_launch
}

resource "aws_subnet" "this_inspection_private_subnets" {
...
resource "aws_route_table_association" "public" {
  count = local.public_subnets_count > 0 ? local.public_subnets_count : 0
  subnet_id = element(aws_subnet.this_inspection_public_subnets[*].id,count.index)
